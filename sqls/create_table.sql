...
CREATE TABLE IF NOT EXISTS `data_warehouse`.`actor_corporation` (
  ...
  `actor_name1` VARCHAR(350) NOT NULL,
  `actor_name2` VARCHAR(350) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `data_warehouse`.`actors_group` (
  ...
  `actor_id` INT NOT NULL,
  `names` LONGTEXT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `data_warehouse`.`review` (
  ...
  `mood` VARCHAR(45) NULL DEFAULT 'Neutral',
  PRIMARY KEY (`id`),
  INDEX `score_index` (`score` ASC) VISIBLE,
  INDEX `mood_index` (`mood` ASC) VISIBLE,
  INDEX `user_scroe_index` (`userId` ASC, `score` ASC) VISIBLE,
  INDEX `user_mood_index` (`userId` ASC, `mood` ASC) VISIBLE)
ENGINE = InnoDB;

...
public interface DateSqlMapper {
    ...
    @Insert("insert into date(year,month,weekday,count,movies,date_str) values(#{year},#{month},#{weekday},1,concat('|',#{movies},'|'),#{date_str})")
    @Options(useGeneratedKeys = true,keyProperty = "date_id")
    Integer InsertDate(Date date);

    @Update("update date set count=count+1,movies=concat(movies,#{movies},'|') where year=#{year} and month=#{month} and weekday=#{weekday}")
